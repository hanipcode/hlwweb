...
(0, _pubnub.subscribe)("43", null, function (res) {
...
exports.generateHeart = generateHeart;
...
var presenceSubscription = new Set();
var messageSubscription = new Set();
...
var connect = exports.connect = function connect() {
  ...
  connection = new Promise(function (resolve, reject) {
    var uuid = identifier();
    var options = Object.assign({}, _config.config.pubnub, { uuid: uuid });
      ...
      status: function status(statusEvent) {
        ...
        pubnub.addListener({
          message: function message() {
            messageSubscription.forEach(function (handler) {
              return handler.apply(undefined, _arguments);
            });
          },
          presence: function presence() {
            presenceSubscription.forEach(handler.apply(undefined, _arguments));
          },
          status: function status(statusEvent) {
            switch (statusEvent.category) {
    ...
    var handshake = function handshake(pubnub) {
      return new Promise(function (resolve, reject) {
        pubnub.time(function (status) {
          if (status.error) {
            reject(new Error("Pubnub service error on handshalke" + status.error));
          } else {
            resolve(pubnub);
          }
        });
    ...
    return handshake(pubnub).then(function () {
      return resolve({ uuid: uuid, pubnub: pubnub });
    }).catch(reject);
  });
...
function subscribe(channel, presenceHandler, messageHandler) {
  presenceSubscription.add(presenceHandler);
  messageSubscription.add(messageHandler);

  connect().then(function (_ref) {
    ...
    return {
      unsubscribe: function unsubscribe() {
        ...
        return connect().then(function (handle) {
          return handle.unsubscribe({ channel: channel });
        });
      }
    };
  });
...
function publishMessage(channel, message) {
  return new Promise(function (resolve, reject) {
    connect().then(function (_ref2) {
      var pubnub = _ref2.pubnub;
...
function generateHeart(broadcaster_id, numberOfTimes) {
  _lodash2.default.range(numberOfTimes).forEach(function () {
